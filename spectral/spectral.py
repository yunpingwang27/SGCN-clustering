...
# from com_modularity import signed_modularity
# comm = pd.read_csv('./input/create/node_comm500.csv',header=None)
...
data = pd.read_csv('./input/edge_list_war1.csv', header=0)
# data = pd.read_csv('./input/create/node_500.csv', header=0)
edge = data.values.tolist()
...
for i, row in data.iterrows():
    if row[2] == 1:
        ...
        A[int(row[0]), int(row[1])] -= 1
        A[int(row[1]), int(row[0])] -= 1
with open('./output/k-mean/adjacency_war1.csv', 'w', newline='') as file: 
...
# n = int(n)
# n = n+1
q = signed_modularity(data,labels,n)
...
with open('./output/spectral/test_spectral_war1.csv', 'w', newline='') as file: 
    writer = csv.writer(file) 
