...
def cluster(outcomm_path,edge_path,embedding_path,nclusters,truecomm_path):
    # args = parameter_parser()
    output_path1 = outcomm_path+'1.csv'
    output_path2 = outcomm_path+'2.csv'
    sys.path.append(os.path.dirname(os.path.abspath(__file__)) + "/..")
    from com_modularity import signed_modularity

    edge_list = pd.read_csv(edge_path,header = 0)
    # 读取数据
    data = pd.read_csv(embedding_path, header=0)
    scaler = StandardScaler()
    data = scaler.fit_transform(data)
    data1 =data[:,1:]
    s = []
    # n = nclusters
    clustering = AgglomerativeClustering(n_clusters=nclusters).fit(data1)
    ...
    kmeans = MiniBatchKMeans(n_clusters=nclusters,batch_size=1024).fit(data1)
    ...
    # nmi值
    if truecomm_path != None:
        comm = pd.read_csv(truecomm_path,header=None)
...
cluster('./output/k-mean/node_500_comm_te','./input/create/node_500.csv','./output/embedding/node_500_te.csv',3,'./input/create/node_comm500.csv')
